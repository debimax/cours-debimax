...
%Dernière modification: 05 oct. 2014 17:46:47
...
{Fondamentaux}
\begin{list}{$\bullet $}{}
\item \textcolor{red}{<<\texttt{Déclaration de variable: }>>}  \code{var=1} sera un entier, \code{var=1.0} (sera un réel),  \code{var="1"} (sera une chaine de caractère), \code{var=[var1,var2,var3]} (sera une liste),  \code{var=(var1,var2,var3)} sera un tuple.
...
\item \textcolor{red}{<<\texttt{Variable saisie au clavier: }>>}
	...
	\code{var=input("texte s'affichant à l'écran et demandant de saisir une variable")}
...
\item \textcolor{red}{<<\texttt{Opérateurs: }>>} \lstinline{ +  -  *  % } (modulo) \lstinline{ //}  (Quotient de la division euclidienne) \quad    \textbf{**} (puissance)  \quad    \textbf{=} (affectation)
...
\item \textcolor{red}{<<\texttt{L = [el1  ,el2 , el3]}>>} créer une liste $L$.
\item \textcolor{red}{<<\texttt{L = [i**2 for i in range(10)]}>>} créer la liste des carrés.\\
...
\item \begin{list}{\ding{212}}{\underline{Fonctions sur les  listes}}
	...
	\end{list}
\item \begin{list}{\ding{212}}{\underline{Modification de liste (méthodes)}}
	...
	\item \textcolor{red}{<<\texttt{L.extend(t)}>>}  concatène la liste $l$ avec le contenu de la liste $t$ sans réaffectation.
...
%ajout d'un élément en fin L=L1+[var] ou L=L1*3 (3concaténations de L1)
...
\item \textcolor{red}{<<\texttt{str.split('sep')}>>}  Retourne une liste des mots de la chaîne, en utilisant sep pour séparateur.\\
 \texttt{'Pablo Neruda'.split(' ')} retourne ['Pablo','Neruda']
...
\texttt{'-'.join(['Pablo','Neruda'])} retourne 'Pablo-Neruda'
\item \textcolor{red}{<<\texttt{str.count(sub[, start[, end]])}>>} Compte le nombre d'occurrences du caractère sub dans str (dans l'intervalle [start ;end]).
\item \textcolor{red}{<<\texttt{str.find(sub[, start[, end]])}>>} Retourne l'indice le plus faible dans la chaîne où \textit{sub} se trouve. Retourne -1 si  \textit{sub} n'est pas trouvé.
\item \textcolor{red}{<<\texttt{ str.index(sub[, start[, end]])}>>}Comme \textit{find()} , mais retourne une ValueError quand la sous-chaîne n'est pas trouvée.
\item \textcolor{red}{<<\texttt{str.rfind(sub[, start[, end]])}>>}Retourne l'indice le plus élevé dans la chaîne \textit{str} où \textit{sub} sous se trouve. Retourne -1 en cas d'echec.
...
Pablo Neruda .replace('a','*') retourne 'P*blo Nerud*'
...
%\item \textcolor{red}{<<\texttt{str.replace(old , new)}>>} Remplacer les bouts de chaine old par un new
%\item \textcolor{red}{<<\texttt{"el".join(L)}>>} Concaténer une liste avec l'élément `el' entre chaque items
\end{list}
\end{Sec}
...
\item \textcolor{red}{<<\texttt{importer un module}>>}
	...
	\begin{list}{\ding{212}}{}
	\item \code{import math}, on sera obligé d'utiliser math. avant d'utiliser un fonction du module  \code{math.sqrt(2)}.
	\item \code{import numpy as np}, np sera alors un racourci du module numpy  \code{np.exp(1)}.
	\item \code{from math import sqrt, sin} (on importe seulement la fonctions \textit{sqrt} et \textit{sin}  du module \textit{math})
...
\item \textcolor{red}{<<\texttt{modules courants}>>} maths, os (système), random, time, tkinter (fenêtres), numpy, matplotlib (graphique), sympy (calcul formel), httplib (connections http) \dots
\item \textcolor{red}{<<\texttt{Exemple}>>}
%\begin{lstlisting}
%def factorielle_rec ( n):
%""" Fonction factorielle , version recursive """
%	if n ==0:
%		return 1
%	else :
...
\item  \textcolor{red}{<<\texttt{fichier = open("monfichier.txt", "r")}>>}  pour ouvrir un fichier en lecture
	...
	Les options sont: \textbf{r} lecture seule,  \textbf{w} écriture, \textbf{a} pour ajouter du texte \textbf{b} binaire
...
%\begin{list}{$\bullet $}{}
\item \textcolor{red}{<<\texttt{\string\w }>>} correspond à tout caractère alphabétique,ce qui est équivalent à [a-zA-Z];
\item \textcolor{red}{<<\texttt{\string\W}>>} correspond à tout ce qui n'est pas un caractère alphabétique ;
...
Dans sympy il faut déclarer les variables symboliques explicitement avec \textbf{Symbol} ou \textbf{S}.
...
\item \textcolor{red}{<<\texttt{pprint(sqrt(12))}>>} pour afficher $\sqrt[]{12}$.
\item \textcolor{red}{<<\texttt{x=Symbol('x')}>>} ou \textcolor{red}{<<\texttt{x=S('x')}>>} définit x comme variable
\item \textcolor{red}{<<\texttt{((x+3)**2).expand()}>>} pour développer $(x+3)^2$
\item \textcolor{red}{<<\texttt{(x**2+2*x-3).factor()}>>} pour factoriser.
\item \textcolor{red}{<<\texttt{(x**2+2*x-3)**2).subs(x, 1)}>>} pour substituer x par 1.
...
\item \textcolor{red}{<<\texttt{solve('x\string^2-2*x-3',x)}>>} pour résoudre $x^2-2*x-3=0$
