...
%Dernière modification: 19 juin 2017 09:04:25
...
Première méthode:
\begin{list}{$\bullet$}{}
\item notebook  (votre navigateur  s'ouvre)  \ding{212} new \ding{212} Indiquez le fichier base.ipy 
\end{list}
...
Indiquer alors pour ouvrir ce fichier l'application c/Programme file32/jupyter/jupyter
\end{list}
...
\begin{enumerate}
\item Avec un logiciel en ligne.

	\begin{list}{$\bullet$}{}
	\item Mes cours : \url{https://github.com/debimax/cours-debimax}

	par exemple pour le 1° cours, on télécharge le fichier \textbf{bases.ipynb} cliquez sur:\\
	\textit{bases.ipynb \ding{212} Raw \ding{212} Enregistrez sous}
	\item Deux fichiers important pour vous aider lorsque vous êtes sur mon git.
\begin{list}{-}{}
...
N'oubliez pas de cliquer sur \textit{Raw} pour les obtenir.
	\item jupyter en ligne :  \url{https://try.jupyter.org/}  puis cliquez sur :\\
	\item Pour juste les visualiser sans pouvoir exécuter le code \url{http://nbviewer.ipython.org/github/debimax/cours-debimax/tree/master/}
	\textit{Upload \ding{212} indiquez votre fichier .ipynb \ding{212} Upload \ding{212} bases.ipynb} 
	\end{list}
\item Depuis votre gestionnaire de fichier.
	\begin{list}{$\bullet$}{}
	\item Cliquez directement sur le fichier ipynb.
	\item Une fenêtre s'ouvre \textit{c:Program File32/pyzo32/jupyter-notebook}
	\item L'adresse du navigateur et en principe \url{localhost:8888}.
	\end{list}
\item Sous windows
Installez par exemple \textbf{pyzo} \url{http://www.pyzo.org/} qui la version utilisé de python pour l'agrégation de mathématiques.

ipython est installé par défaut. %et vous pouvez toujours installer jupiter en tapant dans la console pip install jupyter
\item Sous linux on travaillera avec \textit{geany} ou avec \textit{pyzo}. Je vous conseille cependant pour les test d'utiliser une  jupyter-console (ipython3).
\item Pour booter une clef usb sur un ordinateur il faut modifier l'odre du bios
\end{enumerate}
